...
use MarkupKit\Basic\Node\Flow\AttributedStringNodeParser;
...
class AttributedStringBundleTest extends TestCase
{
    /**
    ...
    {
        $flowNodeParser = new readonly class extends AttributedStringNodeParser {
            protected function createStringBuilderForFlowNode(DOMNode $node, Parser $parser): AttributedStringBuilder
            {
                return new AttributedStringBuilder(
                    preserveWhitespace: $node->nodeName === 'pre',
                    trimWhitespaceAroundAttachments: $node->nodeName === 'p'
                );
            }
        };

        $options = new Options(nodeParsers: [$flowNodeParser, new StringBundle()]);
        self::$parser = new Parser(options: $options);
    }

    private function parseAttributedString(string $html): AttributedString
    {
    ...
    {
        $html = '<pre>This is a text with an image  <img src="https://example.com/image.jpg" alt="Example Image"> embedded in it.</pre>';
        $string = $this->parseAttributedString($html);
        $this->assertCount(3, $string->elements);
    ...
    {
        $html = '<p>This is a text with an image  <img src="https://example.com/image.jpg" alt="Example Image"> embedded in it.</p>';
        $string = $this->parseAttributedString($html);
        $this->assertCount(3, $string->elements);
